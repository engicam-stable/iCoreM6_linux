                                    ...
                                    u8 *buf, int buflen)
{
		...
		printk("crc error: 0x%02x expected, got 0x%02x\n",crc, buf[buflen-1]);
                return false;
        }
...
static irqreturn_t edt_ft5x06_ts_isr(int irq, void *dev_id)
{
        ...
        if (error) {
                ...
                dev_err_ratelimited(dev, "Unable to fetch data, error: %d\n",
                                    error);
#endif
		printk( "Unable to fetch data, error: %d\n",error);
                goto out;
        }
		...
		printk("Unexpected header: %02x%02x%02x!\n",
                                    rdbuf[0], rdbuf[1], rdbuf[2]);
...
static int __init edt_ft5x06_init(void)
{
	return i2c_add_driver(&edt_ft5x06_ts_driver);
}

static void __exit edt_ft5x06_exit(void)
{
	i2c_del_driver(&edt_ft5x06_ts_driver);
}

module_init(edt_ft5x06_init);
module_exit(edt_ft5x06_exit);
